...
import type {
  CreateDiningDto,
  MealTimes,
} from "@/types/dining/dining/dtos/CreateDiningDto";
...
const OpenDiningModal = ({ open, onClose, refetch }: OpenDiningModalProps) => {
  ...
  const handleOnSubmit: SubmitHandler<CreateDiningDto> = async (
    data: CreateDiningDto
  ) => {
    setLoading(true);
    await createDining
      .mutateAsync(
        { ...data },
        {
          onError: () => {
            ...
            setError("openingDate", {
              message: `Ya hay caja de ${data?.mealTime} esta fecha`,
            });
          },
          onSuccess: async () => {
