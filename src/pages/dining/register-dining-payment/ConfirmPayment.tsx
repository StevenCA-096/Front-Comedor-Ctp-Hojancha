...
const ConfirmPayment = ({
  ...
  const [paymentMethod, setPaymentMethod] =
    useState<PaymentMethods>("Efectivo");
  const [montoRecibido, setMontoRecibido] = useState<string>(studentPaymentData?.amountToPay.toString() || "");
  ...
  useEffect(() => {
    if (paymentMethod === "Efectivo" && montoRecibido) {
      ...
      const montoPagar = parseFloat(
        studentPaymentData?.amountToPay.toString() || "0"
      );
  ...
  const handleConfirmPayment: SubmitHandler<DiningStudentFormData> = async (
    data: DiningStudentFormData
  ) => {
    console.log({
      ...data,
                      ...
                      parseFloat(montoRecibido) <
                        ...
                        >
                          ...
                          >
                            ...
                            {(
                              parseFloat(
                                studentPaymentData?.amountToPay.toString() ||
                                  "0"
                              ) - parseFloat(montoRecibido)
                            ).toFixed(0)}
                          </Typography>
